...
namespace WebAPI.StorageClient
{
    public interface IStorageApiClient
    {
        ...
        Task<List<StatementDTO>> GetStatementsByPersonOrDate(string? name, string? startDate, string? endDate);
    ...
    public class StorageApiClient : IStorageApiClient
    {
        ...
        public async Task<List<StatementDTO>> GetStatementsByPersonOrDate(string? names, string? startDate, string? endDate)
        {
            _logger.LogInformation("GetStatementsByPersonOrDate()");
            using var connection = _storageConnection.CreateConnection();
            var response = await connection.GetAsync($"api/statements/lookup?names={names}&startDate={startDate}&endDate={endDate}");
            var statements = await response.Content.ReadFromJsonAsync<StatementDTO[]>();
